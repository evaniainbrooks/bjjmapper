  ...
  describe 'GET index' do
    ...
    context 'with date range' do
      context 'with matching events in date range' do
        ...
        it 'returns events that are within the date range' do
          get :index, { user_id: user.id, format: 'json', start: start_time, end: end_date }
      ...
      context 'with no matching events in date range' do
        it 'returns 204 no content' do
          get :index, { user_id: user.id, format: 'json', start: 99.hours.ago.iso8601, end: 100.hours.ago.iso8601 }
    ...
    context 'without date range' do
      it 'returns bad request' do
        get :index, { user_id: user.id, format: 'json', start: start_time }
